...
app.post("/", (req, res) => {
  ...
  getGoogleLocation(zipcode).then(response => {
...
function getData(lat, long) {
  return rp({
    ...
    for (let i = 0; i < otherdata.response.groups[0].items.length; i++) {
      let location_name =
        otherdata.response.groups[0].items[i].venue.name || "";
      let latitude =
        otherdata.response.groups[0].items[i].venue.location.lat || 0;
      let longitude =
        otherdata.response.groups[0].items[i].venue.location.lng || 0;
      let location_address =
        otherdata.response.groups[0].items[i].venue.location.address || "";
      let city =
        otherdata.response.groups[0].items[i].venue.location.city || "";
      let zipCode =
        otherdata.response.groups[0].items[i].venue.location.postalCode || 0;
      let category_name =
        otherdata.response.groups[0].items[i].venue.categories[0].pluralName ||
        "";
